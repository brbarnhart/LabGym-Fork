...
def sort_examples_from_csv_subject_aware(path_to_examples,out_path,csv_path=None,copy=False):
	'''
	Subject-aware variant of sort_examples_from_csv.

	Supports multi-subject frame_labels (frame, subject_id, behaviors...) and
	parses LabGym example basenames for animal id + frame before matching.
	Falls back to frame-only matching when subject is absent.
	'''
	from LabGym.training.example_sort import sort_examples_from_csv_subject_aware as _sort
	return _sort(path_to_examples,out_path,csv_path=csv_path,copy=copy)


def sort_examples_from_annotations(annotations_path,path_to_examples,out_path,copy=False):
	'''Sort unsorted LabGym examples using a LabGym Behavior Annotator JSON session.'''
	from LabGym.training.example_sort import sort_examples_from_annotations as _sort
	return _sort(annotations_path,path_to_examples,out_path,copy=copy)
