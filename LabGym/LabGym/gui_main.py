...
Set the App Name: To change the name displayed in the Application menu from "Python" to your app's name, you must bundle your script into a .app package using tools like Py2app or Briefcase
		...
		annotate_item = tools_menu.Append(
			wx.ID_ANY,
			'&Behavior Annotator...\tCtrl+Shift+A',
			'Open the PySide6 frame-by-frame behavior annotator',
		)
		...
		self.Bind(wx.EVT_MENU, self.on_open_annotator, annotate_item)
	...
	def on_open_annotator(self, event):
		"""Launch the LabGym Behavior Annotator in a separate process.

		PySide6 and wxPython should not share an event loop in one process,
		so the annotator is started via subprocess.
		"""
		import subprocess

		try:
			# Prefer the console script if installed; fall back to -m
			cmd = [sys.executable, '-m', 'LabGym.annotator']
			subprocess.Popen(cmd, close_fds=True)
			logger.info('Launched LabGym Behavior Annotator: %s', ' '.join(cmd))
		except Exception as exc:  # pragma: no cover - UI path
			logger.exception('Failed to launch Behavior Annotator')
			wx.MessageBox(
				f'Could not launch the Behavior Annotator:\n{exc}',
				'LabGym',
				wx.OK | wx.ICON_ERROR,
			)
