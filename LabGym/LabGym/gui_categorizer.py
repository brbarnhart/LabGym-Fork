...
from .tools import sort_examples_from_csv,sort_examples_from_csv_subject_aware,sort_examples_from_annotations
...
class PanelLv3_SortExamplesCSV(wx.Panel):
	...
	def display_window(self):
		...
		button_sort_subject=wx.Button(panel,label='Sort examples (subject-aware CSV)',size=(300,40))
		button_sort_subject.Bind(wx.EVT_BUTTON,self.sort_fromcsv_subject_aware)
		wx.Button.SetToolTip(button_sort_subject,'Use multi-subject frame_labels (frame, subject_id, behaviors) and parse animal IDs from example filenames.')
		boxsizer.Add(button_sort_subject,0,wx.RIGHT|wx.ALIGN_RIGHT,90)
		boxsizer.Add(0,5,0)

		button_sort_ann=wx.Button(panel,label='Sort examples from annotation session…',size=(300,40))
		button_sort_ann.Bind(wx.EVT_BUTTON,self.sort_from_annotations)
		wx.Button.SetToolTip(button_sort_ann,'Import a LabGym Behavior Annotator .annotations.json session and sort examples by subject + frame.')
		boxsizer.Add(button_sort_ann,0,wx.RIGHT|wx.ALIGN_RIGHT,90)
		boxsizer.Add(0,10,0)
	...
	def sort_fromcsv_subject_aware(self,event):

		if self.path_to_examples is None or self.result_path is None:
			wx.MessageBox('No input / output folder.','Error',wx.OK|wx.ICON_ERROR)
		else:
			counts=sort_examples_from_csv_subject_aware(self.path_to_examples,self.result_path)
			wx.MessageBox('Subject-aware sort finished.\n'+str(counts),'Done',wx.OK|wx.ICON_INFORMATION)


	def sort_from_annotations(self,event):

		if self.path_to_examples is None or self.result_path is None:
			wx.MessageBox('No input / output folder.','Error',wx.OK|wx.ICON_ERROR)
			return
		with wx.FileDialog(self,'Select annotation session JSON','',wildcard='JSON (*.json)|*.json',style=wx.FD_OPEN|wx.FD_FILE_MUST_EXIST) as dlg:
			if dlg.ShowModal()!=wx.ID_OK:
				return
			ann_path=dlg.GetPath()
		try:
			counts=sort_examples_from_annotations(ann_path,self.path_to_examples,self.result_path)
			wx.MessageBox('Annotation sort finished.\n'+str(counts),'Done',wx.OK|wx.ICON_INFORMATION)
...
class PanelLv2_TrainCategorizers(wx.Panel):
	...
	def __init__(self, parent):
		...
		self.label_mode='hard_soft_aux'  # hard_only | hard_soft_aux | soft_primary
		self.lambda_soft=0.4
		self.soft_labels_path=None
	...
	def display_window(self):
		...
		module_labelmode=wx.BoxSizer(wx.HORIZONTAL)
		button_labelmode=wx.Button(panel,label='Specify label mode\n(hard / soft labels)',size=(300,40))
		button_labelmode.Bind(wx.EVT_BUTTON,self.specify_label_mode)
		wx.Button.SetToolTip(button_labelmode,'hard_only uses folder hard labels. hard_soft_aux (default) adds a soft CE/KL term from soft_labels.csv produced by the Behavior Annotator. soft_primary trains mainly on soft targets.')
		self.text_labelmode=wx.StaticText(panel,label='Default: hard_soft_aux (lambda=0.4). Place soft_labels.csv in the prepared examples folder.',style=wx.ALIGN_LEFT|wx.ST_ELLIPSIZE_END)
		module_labelmode.Add(button_labelmode,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		module_labelmode.Add(self.text_labelmode,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(module_labelmode,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0)
	...
	def specify_label_mode(self,event):
		choices=['hard_only','hard_soft_aux (recommended)','soft_primary']
		dialog=wx.SingleChoiceDialog(self,'Select training label mode','Label mode',choices)
		if dialog.ShowModal()==wx.ID_OK:
			sel=dialog.GetStringSelection()
			if sel.startswith('hard_only'):
				self.label_mode='hard_only'
			elif sel.startswith('soft_primary'):
				self.label_mode='soft_primary'
			else:
				self.label_mode='hard_soft_aux'
		dialog.Destroy()
		if self.label_mode!='hard_only':
			dialog=wx.TextEntryDialog(self,'lambda_soft (weight on soft loss)','Soft loss weight',str(self.lambda_soft))
			if dialog.ShowModal()==wx.ID_OK:
				try:
					self.lambda_soft=float(dialog.GetValue())
				except ValueError:
					pass
			dialog.Destroy()
			dialog=wx.MessageDialog(self,'Select a soft_labels.csv file now? (Cancel = auto-detect in training folder)','Soft labels path',wx.YES_NO|wx.ICON_QUESTION)
			if dialog.ShowModal()==wx.ID_YES:
				with wx.FileDialog(self,'soft_labels.csv','',wildcard='CSV (*.csv)|*.csv',style=wx.FD_OPEN|wx.FD_FILE_MUST_EXIST) as fd:
					if fd.ShowModal()==wx.ID_OK:
						self.soft_labels_path=fd.GetPath()
			dialog.Destroy()
		self.text_labelmode.SetLabel(
			f'Mode: {self.label_mode}, lambda_soft={self.lambda_soft}, soft_path={self.soft_labels_path or "auto"}'
		)
	...
	def train_categorizer(self,event):
		...
		if self.data_path is None:
			...
			if do_nothing is False:
				CA=Categorizers()
				CA.label_mode=self.label_mode
				CA.lambda_soft=self.lambda_soft
				if self.animation_analyzer is False:
					if self.behavior_mode>=3:
						self.length=0
						self.std=0
