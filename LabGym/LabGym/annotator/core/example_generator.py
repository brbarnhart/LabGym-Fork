...
class ExampleGenerator:
    def __init__(
        ...
        # None → session.active_subject_id
    ...
    def _bouts_map(self) -> dict:
        from LabGym.annotator.core.data_models import BEHAVIOR_MODE_INTERACTIVE_BASIC

        if int(self.session.behavior_mode) == BEHAVIOR_MODE_INTERACTIVE_BASIC:
            g = self.session.interaction_bouts.setdefault("group", {})
            for beh in self.session.behaviors:
                g.setdefault(beh.name, [])
            return g
        return self.session.bouts_for_subject(self._resolved_subject_id())
    ...
    def build_frame_labels_df(self, open_starts: Optional[dict[str, int]] = None) -> pd.DataFrame:
        ...
        # Completed bouts (active / selected subject)
        bmap = self._bouts_map()
        for name in behaviors:
            for bout in bmap.get(name, []):
    ...
    def export_frame_labels_all_subjects(
        self,
        output_dir: str | Path,
        open_starts_by_subject: Optional[dict] = None,
        combined: bool = True,
    ) -> List[Path]:
        """Write per-subject frame_labels_{id}.csv and optional combined multi-index CSV.

        Combined format columns: frame, subject_id, <behaviors...>
        """
        out_dir = Path(output_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        written: List[Path] = []
        combined_rows = []
        subjects = list(self.session.subjects) or [
            type("S", (), {"subject_id": self._resolved_subject_id()})()
        ]
        for subj in subjects:
            ...
            gen = ExampleGenerator(
                ...
                video_handler=self._video_handler,
                subject_id=sid,
            )
            opens = None
            if open_starts_by_subject:
                opens = open_starts_by_subject.get(sid)
            df = gen.build_frame_labels_df(open_starts=opens)
            path = out_dir / f"frame_labels_subject{sid}.csv"
            df.to_csv(path, index=False)
            written.append(path)
            if combined:
                part = df.copy()
                part.insert(1, "subject_id", sid)
                combined_rows.append(part)
        if combined and combined_rows:
            all_df = pd.concat(combined_rows, ignore_index=True)
            cpath = out_dir / "frame_labels_all_subjects.csv"
            all_df.to_csv(cpath, index=False)
    ...
    def sample_clips(
        ...
        bmap = self._bouts_map()
        sid = self._resolved_subject_id()
        for bname in behaviors:
            bouts = bmap.get(bname, [])
            for bout_idx, bout in enumerate(bouts):
                ...
                if mode == "full_bout":
                    ...
                    if b_len < clip_length:
                        ...
                        if cend >= cstart:
                            specs.append(
                                ClipSpec(bname, cstart, cend, bout_idx, sid)
                            )
                    else:
                        ...
                        if end >= start:
                            specs.append(
                                ClipSpec(bname, start, end, bout_idx, sid)
                            )
                    continue
                ...
                if mode == "full_bout_coverage":
                    ...
                    if b_len <= clip_length:
                        ...
                        while pos + clip_length <= L:
                            ...
                            e = s + clip_length - 1
                            specs.append(ClipSpec(bname, s, e, bout_idx, sid))
                            pos += clip_length
                        ...
                        if not specs or specs[-1].end_frame < fe:
                            specs.append(
                                ClipSpec(bname, fs, fe, bout_idx, sid)
                            )
                    continue
                ...
                if b_len < clip_length:
                    ...
                    specs.append(ClipSpec(bname, start, end, bout_idx, sid))
                    continue
                ...
                if mode == "centered":
                    ...
                    for _ in range(min(n_random, max(1, b_len - clip_length + 1))):
                        ...
                        specs.append(
                            ClipSpec(bname, int(start), int(end), bout_idx, sid)
                        )
                    ...
                    while pos + clip_length - 1 <= bout.end_frame:
                        specs.append(
                            ClipSpec(
                                bname, pos, pos + clip_length - 1, bout_idx, sid
                            )
                        )
                        pos += step
                        clip_count += 1
    ...
    def _write_clip_specs(
        ...
        filename_template: str = (
            "{video}_sub{subject}_{behavior}_bout{bout:03d}_f{start:05d}-{end:05d}.mp4"
        ),
        fourcc: str = "mp4v",
    ) -> List[Path]:
        ...
        for behavior, blist in by_behavior.items():
            ...
            for spec in blist:
                ...
                fname = filename_template.format(
                    video=video_id,
                    subject=getattr(spec, "subject_id", self._resolved_subject_id()),
                    behavior=behavior,
