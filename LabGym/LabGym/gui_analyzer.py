...
from .analysis_presets import (
	...
	load_preset,
	panel_to_preset,
	refresh_analyze_behaviors_labels,
	save_preset,
)
...
class PanelLv2_AnalyzeBehaviors(wx.Panel):
	...
	def __init__(self, parent):
		...
		self.id_review_enabled=False # contact-risk ID review + training pack export (detector path)
		# Contact detector knobs (used when id_review_enabled); match ContactDetectorConfig defaults
		self.id_review_contact_distance_factor=1.5
		self.id_review_min_contact_frames=3
		self.id_review_gap_bridge_frames=2
	...
	def display_window(self):
		...
		module_id_review=wx.BoxSizer(wx.VERTICAL)
		self.checkbox_id_review=wx.CheckBox(
			panel,
			label='Review contact ID risk (export training pack) — detector multi-animal only',
		)
		self.checkbox_id_review.SetValue(False)
		wx.CheckBox.SetToolTip(
			self.checkbox_id_review,
			'After tracking, open a full-video timeline: risk bands mark likely ID-switch regions; '
			'you place timestamps where switches actually happen. Exports switches/crops under '
			'results/id_review/ for training. Recommended for multi-mouse videos with markings.',
		)
		self.checkbox_id_review.Bind(wx.EVT_CHECKBOX,self.on_toggle_id_review)
		module_id_review.Add(self.checkbox_id_review,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)

		# Contact detection parameters (enabled when checkbox is on)
		param_row=wx.BoxSizer(wx.HORIZONTAL)
		param_row.Add(wx.StaticText(panel,label='Contact params:'),0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT|wx.RIGHT,10)

		param_row.Add(wx.StaticText(panel,label='distance×size'),0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT,8)
		self.spin_contact_dist=wx.SpinCtrlDouble(panel,min=0.1,max=10.0,inc=0.1,initial=1.5,size=(80,-1))
		self.spin_contact_dist.SetDigits(2)
		wx.SpinCtrlDouble.SetToolTip(
			self.spin_contact_dist,
			'Centers closer than this × median animal height count as contact. '
			'Lower = tighter / fewer long bouts (better for ending contact before chase). '
			'Higher = more proximity flagged as contact.',
		)
		param_row.Add(self.spin_contact_dist,0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT|wx.RIGHT,4)

		param_row.Add(wx.StaticText(panel,label='min frames'),0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT,8)
		self.spin_min_contact=wx.SpinCtrl(panel,min=1,max=300,initial=3,size=(70,-1))
		wx.SpinCtrl.SetToolTip(
			self.spin_min_contact,
			'Minimum consecutive contact frames to create an event. '
			'Raise to ignore brief near-misses; lower to catch short wrestles.',
		)
		param_row.Add(self.spin_min_contact,0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT|wx.RIGHT,4)

		param_row.Add(wx.StaticText(panel,label='gap bridge'),0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT,8)
		self.spin_gap_bridge=wx.SpinCtrl(panel,min=0,max=60,initial=2,size=(70,-1))
		wx.SpinCtrl.SetToolTip(
			self.spin_gap_bridge,
			'Bridge non-contact gaps of this many frames inside a bout. '
			'0–1 splits wrestle/chase more often; higher glues brief separations into one event.',
		)
		param_row.Add(self.spin_gap_bridge,0,wx.ALIGN_CENTER_VERTICAL|wx.LEFT|wx.RIGHT,4)

		module_id_review.Add(param_row,0,wx.TOP|wx.EXPAND,4)
		self._set_id_review_params_enabled(False)
		boxsizer.Add(module_id_review,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0)

		module_preset=wx.BoxSizer(wx.HORIZONTAL)
		button_save_preset=wx.Button(panel,label='Save analysis parameters…',size=(200,36))
		button_save_preset.Bind(wx.EVT_BUTTON,self.save_analysis_parameters)
		wx.Button.SetToolTip(
			button_save_preset,
			'Save the current Analyze Behaviors settings (detection method, paths, timing, '
			'ID review knobs, etc.) to a JSON file for reuse.',
		)
		button_load_preset=wx.Button(panel,label='Load analysis parameters…',size=(200,36))
		button_load_preset.Bind(wx.EVT_BUTTON,self.load_analysis_parameters)
		wx.Button.SetToolTip(
			button_load_preset,
			'Load a previously saved Analyze Behaviors JSON preset and restore the UI. '
			'Missing paths are reported as warnings; you can still re-select files after load.',
		)
		self.text_preset=wx.StaticText(panel,label='No preset loaded.',style=wx.ALIGN_LEFT|wx.ST_ELLIPSIZE_END)
		module_preset.Add(button_save_preset,0,wx.LEFT|wx.RIGHT,10)
		module_preset.Add(button_load_preset,0,wx.LEFT|wx.RIGHT,10)
		module_preset.Add(self.text_preset,0,wx.LEFT|wx.RIGHT|wx.ALIGN_CENTER_VERTICAL,10)
		boxsizer.Add(module_preset,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0)
	...
	def on_toggle_id_review(self,event):
		self.id_review_enabled=bool(self.checkbox_id_review.GetValue())
		self._set_id_review_params_enabled(self.id_review_enabled)


	def save_analysis_parameters(self,event):
		'''Save current Analyze Behaviors settings to a JSON preset file.'''
		wildcard='LabGym analysis preset (*.json)|*.json'
		...
		if not path.lower().endswith('.json'):
			path=path+'.json'
		try:
			preset=panel_to_preset(self)
			# Optional short description
			dlg=wx.TextEntryDialog(
				self,
				'Optional description for this preset (what this pipeline is for):',
				'Preset description',
				'',
			)
			if dlg.ShowModal()==wx.ID_OK:
				preset['description']=dlg.GetValue().strip()
			dlg.Destroy()
			save_preset(path,preset)
			self.text_preset.SetLabel('Saved: '+path)
			wx.MessageBox(
				'Analysis parameters saved to:\n'+path,
				'Saved',
				wx.OK|wx.ICON_INFORMATION,
			)
		except Exception as exc:
			logger.exception('Failed to save analysis preset')
			wx.MessageBox(f'Failed to save preset:\n{exc}','Error',wx.OK|wx.ICON_ERROR)


	def load_analysis_parameters(self,event):
		'''Load Analyze Behaviors settings from a JSON preset file.'''
		wildcard='LabGym analysis preset (*.json)|*.json'
		dialog=wx.FileDialog(
			self,
		...
		try:
			...
			warnings=apply_preset_to_panel(self,preset)
			refresh_analyze_behaviors_labels(self)
			desc=preset.get('description') or ''
			label='Loaded: '+path
			if desc:
				label+=' — '+desc
			self.text_preset.SetLabel(label)
			if warnings:
				wx.MessageBox(
					'Preset loaded with warnings:\n\n- '
					+'\n- '.join(warnings)
					+'\n\nYou can re-select any missing paths before running analysis.',
					'Loaded with warnings',
					wx.OK|wx.ICON_WARNING,
				)
			else:
				wx.MessageBox(
					'Analysis parameters loaded from:\n'+path,
					'Loaded',
					wx.OK|wx.ICON_INFORMATION,
				)
		except Exception as exc:
			logger.exception('Failed to load analysis preset')
			wx.MessageBox(f'Failed to load preset:\n{exc}','Error',wx.OK|wx.ICON_ERROR)


	def _set_id_review_params_enabled(self,enabled):
		for ctrl in (
			getattr(self,'spin_contact_dist',None),
			getattr(self,'spin_min_contact',None),
			getattr(self,'spin_gap_bridge',None),
		):
			if ctrl is not None:
				ctrl.Enable(bool(enabled))


	def _read_id_review_config(self):
		'''Build ContactDetectorConfig from Analyze Behaviors UI controls.'''
		from .id_review import ContactDetectorConfig
		self.id_review_contact_distance_factor=float(self.spin_contact_dist.GetValue())
		self.id_review_min_contact_frames=int(self.spin_min_contact.GetValue())
		self.id_review_gap_bridge_frames=int(self.spin_gap_bridge.GetValue())
		return ContactDetectorConfig(
			contact_distance_factor=self.id_review_contact_distance_factor,
			min_contact_frames=self.id_review_min_contact_frames,
			gap_bridge_frames=self.id_review_gap_bridge_frames,
		)


	def _run_id_review(self,analyzer):
		'''Export contact-risk pack, open review dialog, apply Keep/Swap decisions.'''
		import traceback
		from .id_review import run_id_review_pipeline
		from .gui_id_review import run_review_dialog

		config=self._read_id_review_config()
		print(
			'ID review config: '
			f'distance_factor={config.contact_distance_factor}, '
			f'min_contact_frames={config.min_contact_frames}, '
			f'gap_bridge_frames={config.gap_bridge_frames}',
			flush=True,
		)
		# Prefer the top-level frame as dialog parent (more reliable than nested panels)
		parent=wx.GetTopLevelParent(self) or self

		def review_callback(out_dir,events):
			# Full-video timeline: risk bands optional for navigation; still open if none
			if not events:
				wx.MessageBox(
					'No automatic contact-risk bands detected.\n'
					'You can still scrub the full video and mark ID switches manually.\n'
					'(Try raising contact distance×size or lowering min frames for more bands.)',
					'ID review',
					wx.OK|wx.ICON_INFORMATION,
				)
			# Dialog writes switches.jsonl on Done; pipeline loads them for apply.
			run_review_dialog(parent,out_dir,events=events)
			return []

		print('Starting full-video ID review...', flush=True)
		try:
			out_dir,events,decisions=run_id_review_pipeline(
				analyzer,
				config=config,
				review_callback=review_callback,
				extract_samples=False,  # timeline exports samples per switch marker
				auto_load_existing_decisions=True,
			)
			msg=(
				f'ID review pack: {out_dir}\n'
				f'Contact events: {len(events)}\n'
				f'Decisions on disk: {len(decisions)}'
			)
			print(msg, flush=True)
			if hasattr(analyzer,'log'):
				analyzer.log.append(msg)
		except Exception as exc:
			traceback.print_exc()
			wx.MessageBox(
				f'ID review failed (analysis will continue without remaps):\n{exc}',
				'ID review error',
				wx.OK|wx.ICON_WARNING,
			)
			...
			if self.behavior_mode==3:
				...
				for i in self.path_to_videos:
					...
					if self.use_detector is False:
						...
						if self.behavior_mode!=1:
							AAD.craft_data()
							if self.id_review_enabled and self.behavior_mode!=1:
								self._run_id_review(AAD)
						if self.path_to_categorizer is not None:
