...
from core.data_models import AnnotationSession
from ui.video_display import VideoDisplayWidget
from ui.playback_controls import PlaybackControls
from ui.behavior_palette import BehaviorPalette
from ui.timeline_widget import TimelineWidget
from ui.export_dialog import ExportDialog
from ui.bout_list import BoutListWidget


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Behavior Video Annotator")
        ...
        self._settings = QSettings("behavior_video_annotator", "main")
    ...
    def _build_ui(self):
        ...
        # Central: video + palette on right
        ...
        center_layout.addWidget(self.palette_container, 1)
    ...
    def _toggle_bout_for_behavior(self, name: str):
        if not self.manager or not self.video.is_loaded:
            return
        try:
            action, bout = self.manager.toggle_bout(name, self._current_frame)
            if self.palette:
    ...
    def open_video(self):
        ...
        try:
            ...
            if self.manager:
                open_names = self.manager.get_open_behaviors_at_frame(0)
                annotated = self.manager.get_annotated_behaviors_at_frame(0)
                self.video_widget.show_frame(
                    frame,
                    open_behaviors=open_names,
                    annotated_behaviors=annotated,
            ...
            if auto_loaded:
                self.statusBar().showMessage(
                    ...
                    f"Auto-loaded annotations: {ann_path.name}"
                )
            else:
                self.statusBar().showMessage(
                    f"Loaded: {Path(path).name}  |  {meta.total_frames} frames @ {meta.fps:.2f} fps   |  "
                    "Use hotkeys (1,2..) or double-click behaviors to annotate"
                )
            self.setWindowTitle(f"Behavior Video Annotator — {Path(path).name}")
            self._update_undo_action()
            if self.bout_list_dialog is not None:
    ...
    def show_metrics(self):
        ...
        from ui.metrics_panel import MetricsDialog
    ...
    def export_frame_labels_only(self):
        ...
        default = str(Path(self.video.metadata.path).with_name("frame_labels.csv"))
        path, _ = QFileDialog.getSaveFileName(self, "Export frame_labels.csv", default, "CSV (*.csv)")
        if not path:
            return
        try:
            # Close any still-open bouts at the current frame so they are captured in labels
            self.manager.close_all_open_bouts(self._current_frame)
            gen = ExampleGenerator(self.manager.session, video_path=self.video.metadata.path)
            ...
            QMessageBox.information(self, "Exported", f"frame_labels.csv written to:\n{out}")
        except Exception as e:
